...
int main(int argc, char **argv) {
  ...
  for (uint32_t i = 0; i < threads_num; i++) {
    if (pthread_create(&threads[i], NULL, ThreadSum, (void *)&args[i])) {//Новый поток создаётся с помощью функции pthread_create
    /*
...
завершения сохраняет id потока. pthread_attr_t – атрибуты потока. 
...
arg – это аргументы, которые будут переданы функции.
  ...
  for (uint32_t i = 0; i < threads_num; i++) {
    int sum = 0;
    pthread_join(threads[i], (void **)&sum);//используется для ожидания завершения потока
    total_sum += sum;
  }
  free(array);
  printf("Total: %d\n", total_sum);
